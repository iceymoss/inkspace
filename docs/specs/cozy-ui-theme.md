...
- Phase 3 验收时不开放 `swiss`；Phase 4 已按独立规范开放，未知主题继续回退 `magazine`。
...
- [x] Phase 3 验收时 `swiss` 返回 400；Phase 4 已按独立规范开放，未知值、空主题和未知明暗继续返回 400。
- [ ] Cozy PUT 后 GET 一致，用户之间偏好不污染，未登录请求返回 401。

### 前端自动化

- [x] 注册表为 `available` 且 stylesheet 为 `cozy.css`；Phase 4 已另行开放 Swiss。
...
- [x] Appearance 的 Cozy 卡和独立 Live Proof 已开放；Phase 4 已另行开放 Swiss。
...
2. 默认仍为 `magazine + system`；`magazine`、`terminal` 无回归；Swiss 后续已在 Phase 4 开放。
