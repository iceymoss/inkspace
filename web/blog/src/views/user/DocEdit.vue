...
import { ArrowLeft, Clock, MoreFilled, Share } from '@element-plus/icons-vue'
...
const tags = ref([])
const blogForm = reactive({ category_id: null, tag_ids: [], summary: '', cover: '' })
...
const saveStateText = computed(() => {
  ...
  if (titleDirty.value && contentDirty.value) return '标题和正文有未保存更改'
  if (titleDirty.value) return contentAutosaved.value ? '正文已自动保存，标题尚未保存' : '标题尚未保存'
  if (contentDirty.value) return '正文有未保存更改'
...
async function openBlogPublish() {
  publishing.value = true
  try {
    const requests = [api.get('/categories'), api.get('/tags')]
    if (form.article_id) requests.push(api.get(`/articles/${form.article_id}/edit`))
    const [categoryResponse, tagResponse, articleResponse] = await Promise.all(requests)
    categories.value = Array.isArray(categoryResponse.data) ? categoryResponse.data : categoryResponse.data?.list || []
    tags.value = Array.isArray(tagResponse.data) ? tagResponse.data : tagResponse.data?.list || []
    const article = articleResponse?.data
    Object.assign(blogForm, {
      category_id: article?.category_id || categories.value[0]?.id || null,
...
async function publishToBlog() {
  if (!blogForm.category_id) return ElMessage.warning('请选择博客分类')
  if (dirty.value || !form.title.trim()) {
    const saved = await manualSave(false)
    if (!saved) return
  }
  publishing.value = true
  const wasPublished = Boolean(form.article_id)
  try {
    const response = await api.post(`/docs/${docId}/publish-to-blog`, {
      category_id: blogForm.category_id,
      tag_ids: blogForm.tag_ids,
      summary: blogForm.summary,
      cover: blogForm.cover
    })
    form.article_id = response.data.id
    blogDialog.value = false
    ElMessage.success(wasPublished ? '博客文章已同步' : '已发布到博客')
  } finally {
    publishing.value = false
  }
}
...
function handleMoreCommand(command) {
  if (command === 'versions') openVersions()
  if (command === 'shares') openShares()
}
...
async function createShare() {
  ...
  try {
    const response = await api.post(`/docs/${docId}/shares`, {
    ...
    await fetchShares()
    if (response.data?.token) await copyShare(response.data.token, '分享链接已创建并复制')
    else ElMessage.success('分享链接已创建')
  } finally {
...
async function copyShare(token, successMessage = '链接已复制') {
  try {
...
.desktop-secondary-actions .el-button + .el-button { margin-left: 0; }
...
.blog-publish-form { margin-top: 18px; }
@media (max-width: 760px) { .editor-header { align-items: flex-start; flex-direction: column; } .header-actions { width: 100%; justify-content: flex-end; gap: 7px; } .header-actions > .el-button { margin-left: 0; } .desktop-secondary-actions { display: none; } .mobile-more { display: inline-flex; } .editor-surface { padding: 12px; } .share-create { align-items: stretch; flex-direction: column; } .share-create :deep(.el-date-editor) { width: 100% !important; } .share-row { align-items: flex-start; flex-direction: column; } .share-actions { width: 100%; justify-content: flex-end; } .version-detail-header { align-items: flex-start; flex-direction: column; } }
@media (max-width: 460px) { .header-actions { flex-wrap: wrap; } .header-actions > .el-button:last-child { flex: 1; } }
</style>
